 ...
 *
 * All files must be in binary format.
*/
...
int main()
{
  ...
  strcpy(filename_out,"initial_mpi.inp");
  SplitDomain(filename_in,filename_out);

  strcpy(filename_in ,"exact.inp"    );
  strcpy(filename_out,"exact_mpi.inp");
  SplitDomain(filename_in,filename_out);
...
void SplitDomain(char *fnamein, char *fnameout)
{
  ...
  if (!in) {
    ...
    if (!strcmp(word, "begin")){
        ...
        if (!strcmp(word, "ndims")) {
          ...
          int i;
          if (!dim_global) {
            fprintf(stderr,"Error: dim_global not allocated.\n");
          ...
          int i;
          if (!iproc) {
            fprintf(stderr,"Error: iproc not allocated.\n");
  ...
  if (strcmp(input_mode,"mpi-io")) {
    printf("Error: input_mode is not \"mpi-io\".\n");
    return;
  }
