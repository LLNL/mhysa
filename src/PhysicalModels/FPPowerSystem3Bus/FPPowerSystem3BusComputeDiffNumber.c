...
double FPPowerSystem3BusComputeDiffNumber(void *s,void *m,double dt,double t)
{
  ...
  while (!done) {
    double dxinv[ndims],dissp[ndims];
    ...
    FPPowerSystem3BusDissipationFunction(0,params,t,dissp);

    double local_diff[ndims];
    local_diff[0] = absolute(dissp[0]) * dt * dxinv[0] * dxinv[0];
    local_diff[1] = absolute(dissp[1]) * dt * dxinv[1] * dxinv[1];
    local_diff[2] = absolute(dissp[2]) * dt * dxinv[2] * dxinv[2];
    local_diff[3] = absolute(dissp[3]) * dt * dxinv[3] * dxinv[3];
