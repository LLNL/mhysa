...
int NavierStokes3DUpwindRoe(double *fI,double *fL,double *fR,double *uL,double *uR,double *u,int dir,void *s,double t)
{
  ...
  while (!done) {
    ...
    for (index_inter[dir] = 0; index_inter[dir] < bounds_inter[dir]; index_inter[dir]++) {
      ...
      ierr = NavierStokes3DEigenvalues       (&uavg[0],D,param,dir); CHECKERR(ierr);
      ierr = NavierStokes3DLeftEigenvectors  (&uavg[0],L,param,dir); CHECKERR(ierr);
      ierr = NavierStokes3DRightEigenvectors (&uavg[0],R,param,dir); CHECKERR(ierr);
