...
int Numa3DSplitFlux1(double *f,double *u,int dir,void *s,double t)
{
  HyPar  *solver = (HyPar*)   s;
  Numa3D *param  = (Numa3D*) solver->physics;
  int     i;
  ...
  while (!done) {
    int p; _ArrayIndex1DWO_(ndims,dim,index,offset,ghosts,p);
    double drho,uvel,vvel,wvel,dT,dP,rho0,T0,P0;
    double fluxS[_MODEL_NVARS_], fluxL[_MODEL_NVARS_];
    ...
    _Numa3DGetFlowVars_     ((u+_MODEL_NVARS_*p),drho,uvel,vvel,wvel,dT,rho0);
    _Numa3DComputePressure_ (param,T0,dT,P0,dP);
    _Numa3DSetFlux_         (fluxS,dir,drho,uvel,vvel,wvel,dT,dP,rho0,T0);
    _Numa3DSetLinearFlux_   (fluxL,dir,drho,uvel,vvel,wvel,dT,dP,rho0,T0);
    _ArraySubtract1D_       ((f+_MODEL_NVARS_*p),fluxS,fluxL,_MODEL_NVARS_);
...
int Numa3DSplitFlux2(double *f,double *u,int dir,void *s,double t)
{
  HyPar  *solver = (HyPar*)   s;
  Numa3D *param  = (Numa3D*) solver->physics;
  int     i;
