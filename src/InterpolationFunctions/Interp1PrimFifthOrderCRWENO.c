...
int Interp1PrimFifthOrderCRWENO_2(double *fI,double *fC,double *u,double *x,int upw,int dir,void *s,void *m)
{
  ...
  /* calculate total number of tridiagonal systems to solve */
  Nsys = bounds_outer[0]*bounds_outer[1]*nvars;
  ...
  done = 0; int index_outer[_NDIMS_] = {0,0}, indexC[_NDIMS_], indexI[_NDIMS_];
  sys = 0;
  while (!done) {
  ...
  /* save the solution to fI */
  done = 0; index_outer[0] = index_outer[1] = 0;
  sys = 0;
  while (!done) {
    _ArrayCopy1D2_(index_outer,indexI,_NDIMS_);
    for (indexI[dir] = 0; indexI[dir] < dim[dir]+1; indexI[dir]++) {
      int p; _ArrayIndex1D2_(_NDIMS_,bounds_inter,indexI,0,p);
      int v; for (v=0; v<nvars; v++) fI[nvars*p+v] = R[sys*nvars+v+Nsys*indexI[dir]];
    }
    _ArrayIncrementIndex_(_NDIMS_,bounds_outer,index_outer,done);
    sys++;
...
int Interp1PrimFifthOrderCRWENO_3(double *fI,double *fC,double *u,double *x,int upw,int dir,void *s,void *m)
{
  ...
  /* calculate total number of tridiagonal systems to solve */
  Nsys = bounds_outer[0]*bounds_outer[1]*bounds_outer[2]*nvars;
  ...
  done = 0; int index_outer[_NDIMS_] = {0,0,0}, indexC[_NDIMS_], indexI[_NDIMS_];
  sys = 0;
  while (!done) {
  ...
  /* save the solution to fI */
  done = 0; index_outer[0] = index_outer[1] = index_outer[2] = 0;
  sys = 0;
  while (!done) {
    _ArrayCopy1D3_(index_outer,indexI,_NDIMS_);
    for (indexI[dir] = 0; indexI[dir] < dim[dir]+1; indexI[dir]++) {
      int p; _ArrayIndex1D3_(_NDIMS_,bounds_inter,indexI,0,p);
      int v; for (v=0; v<nvars; v++) fI[nvars*p+v] = R[sys*nvars+v+Nsys*indexI[dir]];
    }
    _ArrayIncrementIndex_(_NDIMS_,bounds_outer,index_outer,done);
    sys++;
