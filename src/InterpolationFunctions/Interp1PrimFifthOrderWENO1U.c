...
/* 
  Fifth order WENO-1U interpolation (uniform grid)
*/

#undef  _MINIMUM_GHOSTS_
#define _MINIMUM_GHOSTS_ 3

int Interp1PrimFifthOrderWENO1U(double *fI,double *fC,double *u,double *x,int upw,int dir,void *s,void *m,int uflag)
{
...
#pragma omp parallel for schedule(auto) default(shared) private(i,index_outer,indexC,indexI)
  for (i=0; i<N_outer; i++) {
    ...
    for (indexI[dir] = 0; indexI[dir] < dim[dir]+1; indexI[dir]++) {
      ...
      /* calculate the WENO approximation */
      double fweno[nvars];
      _ArrayMultiply3Add1D_(fweno,w1,f1,w2,f2,w3,f3,nvars);

      /* calculate the hybridization parameter */
      double phi[nvars];
      int v;
      for (v=0; v<nvars; v++) {
        double psi = min3(w1[v],w2[v],w3[v])/max3(w1[v],w2[v],w3[v]);
        if (psi < weno->tol)  phi[v] = 0.0;
        else                  phi[v] = 1.0;
      }

      /* calculate final approximation */
      _ArrayConvexCombination1D_((fI+p*nvars),phi,fweno,fm1,nvars);
    }
  }
