...
int main()
{
  ...
  if (ndims != 1) {
	...
	double *x, *rho1,*rho2,*rhou,*e;
	x    = (double*) calloc (NI, sizeof(double));
	rho1  = (double*) calloc (NI, sizeof(double));
	rho2  = (double*) calloc (NI, sizeof(double));
  ...
  double mass_fraction_1 = 0.3,
    ...
    rho1[i]  = mass_fraction_1*RHO;
    rho2[i]  = mass_fraction_2*RHO;
    rhou[i] = RHO*U;
  ...
  if (!strcmp(ip_file_type,"ascii")) {
    ...
    fprintf(out,"\n");
	  for (i = 0; i < NI; i++)	fprintf(out,"%lf ",rho1[i]);						
    fprintf(out,"\n");
	  for (i = 0; i < NI; i++)	fprintf(out,"%lf ",rho2[i]);						
    fprintf(out,"\n");
