...
cmd = ['gcc ',hypar_path, ...
    '/Examples/1D/ShallowWater1D/SmallPerturbation/aux/init.c -lm ', ...
    '-o INIT'];
system(cmd);
...
[~,~,~,~,~,~,~,~,~,hyp_flux_split,~,par_type,par_scheme,~, ...
 ...
 output_mode,n_io,~,~,~,~,~,~,~, ...
 mapped,borges,yc,nl,~,p,rc,xi,wtol,lutype,norm,maxiter,atol,rtol, ...
 verbose] = SetDefaults();
...
perturb = 0.2;
% set the epsilon parameter in WENO5 (if using WENO5) to
% something less than perturb^2
eps = 1e-6;
...
init_exec = './INIT < perturb.inp > init.log 2>&1';
...
% write a small file to provide the perturbation strength
fid = fopen('perturb.inp','w');
fprintf(fid,'%1.16e\n',perturb);
fclose(fid);
...
grid on;
figure(2);
plot(xcoord,huinit,'-r','linewidth',2);
hold on;
plot(xcoord,hu,'bo','linewidth',1,'MarkerSize',8);
hold on;
xlabel('x','FontName','Times','FontSize',20,'FontWeight','normal');
ylabel('Discharge','FontName','Times','FontSize',20,'FontWeight','normal');
set(gca,'FontSize',14,'FontName','Times');
title(title_str);
legend('Initial   ','Final     ','Location','NorthWest');
axis auto;
