...
cmd = ['gcc ',hypar_path, ...
    '/Examples/1D/ShallowWater1D/DamBreakingRectangularBump/aux/init.c -lm ', ...
    '-o INIT'];
system(cmd);
...
xcoord = data(:,2);
hinit = data(:,3);
% read in the solution at 15s
data = load('op_00001.dat');
h15 = data(:,3);
% read in the solution at 60s
data = load('op_00004.dat');
h60 = data(:,3);
...
% plot the solutions at 15s
figure(1);
plot(xcoord,hinit+b,'-r','linewidth',2);
hold on;
plot(xcoord,h15+b,'bo','linewidth',1,'MarkerSize',8);
hold on;
...
title(title_str);
legend('Initial   ','Final     ','Topography','Location','SouthWest');
axis auto;
grid on;
% plot the solutions at 60s
figure(2);
plot(xcoord,hinit+b,'-r','linewidth',2);
hold on;
plot(xcoord,h60+b,'bo','linewidth',1,'MarkerSize',8);
hold on;
plot(xcoord,b,'-k','linewidth',1,'MarkerSize',8);
hold on;
xlabel('x','FontName','Times','FontSize',20,'FontWeight','normal');
