...
cmd = ['gcc ',hypar_path, ...
    '/Examples/2D/NavierStokes2D/RisingThermalBubble/aux/init.c ', ...
    '-lm -o INIT'];
system(cmd);
% Compile the code to postprocess the output
cmd = ['gcc ',hypar_path, ...
    '/Examples/2D/NavierStokes2D/RisingThermalBubble/aux/PostProcess.c ', ...
    '-lm -o PP'];
system(cmd);
...
HB        = 2;                    % type of hydrostatic balance
BV        = 0.01;                 % Brunt-Vaisala frequency
GasConst  = 287.058;              % Universal gas constant

% other options
cons_check      = 'no';
screen_op_iter  = int32((t_final/100)/dt);
op_format       = 'binary';
op_overwrite    = 'no';
file_op_iter    = int32((t_final/20)/dt);
ip_type         = 'binary';
...
bcdim     = [0; 0; 1; 1;];
face      = [1; -1; 1; -1];
limits    = [0 0 0 1000; 0 0 0 1000; 0 1000 0 0; 0 1000 0 0];
WallVel1  = '0.0  0.0';
WallVel2  = '0.0  0.0';
WallVel3  = '0.0  0.0';
WallVel4  = '0.0  0.0';
...
WriteBoundaryInp(nb,bctype,bcdim,face,limits,WallVel1,WallVel2,WallVel3,WallVel4);
