...
int main()
{
  ...
  /* calculating location of initial Dirac */
  x0 = 0.89*asin(Pm_avg/Pmax);
  y0 = 0.99*omegaS;
  ...
  printf("Nearest grid point to initial Dirac is (%d,%d) -> (%1.16E, %1.16E)\n",imin,jmin,x[imin],y[jmin]);
  if ((imin > -1) && (jmin > -1)) {
    x0 = x[imin];
    y0 = y[jmin];
    /* locate the dirac at this grid point */
    int p = NI*jmin + imin;
    u[p] = 1.0/(dx*dy);
